...
## Phase 22 — TankOS i18n (16 languages, VPS-hosted packs) ✅

- **I18nManager** (`core/i18n.py`) — language switching, `t()` lookup, VPS
  fetch + local cache, offline-first fallback, Qt widget-tree translation.
- **VPS hosting** — packs generated by `scripts/langgen.py` → nginx
  `/var/www/html/lang/` (tailnet + public IP, HTTP 200 verified).
- **GUI** — Settings → 🌐 Language dropdown + sync button; dock labels
  translate live; language persisted and applied at boot.
- Tracker: `docs/TANKOS_I18N_PLAN.md` · Screenshots `69`, `70`.
